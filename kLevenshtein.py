...
'''
def kLevenshtein_lk(A, B, sigma, epsilon):
    ...
    l=min(la,lb)
    lmx=max(la,lb)
    ins_delCost = (np.exp(-1.0/sigma)+epsilon)/(3.0*(1+epsilon));
    matchCost = 1.0/3.0;
    substCost = (np.exp(-1.0/sigma)+epsilon)/(3.0*(1+epsilon));
...
def kLevenshtein(A, B, sigma = .1, epsilon = 1e-3):
    A = str2array(A)
    B = str2array(B)
    return kLevenshtein_lk(A,B,sigma, epsilon)
